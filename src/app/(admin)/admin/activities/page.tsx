...
export default function ActivitiesPage() {
  ...
  useEffect(() => {
    fetch("/api/hospitals")
      .then(r => r.ok ? r.json() : [])
      .then((d: unknown) => setAccounts(Array.isArray(d) ? d : []))
      .catch(() => {});
  }, []);
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        ...
        <button
          ...
          disabled={relinking}
          title="Re-link activities that have no account connected"
          style={{ background: "rgba(255,255,255,0.05)", border: `1px solid rgba(201,168,76,0.2)`, color: relinking ? "rgba(237,228,207,0.3)" : "rgba(237,228,207,0.6)", fontWeight: 700, fontSize: "0.82rem", borderRadius: 10, padding: "10px 14px", cursor: relinking ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}
        >
          {relinking ? "Linking…" : "🔗 Re-link Accounts"}
        </button>
        <button
          onClick={load}
          disabled={loading}
        ...
        {selected.size > 0 && (
          <button
            ...
            style={{ background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.35)", color: bulkDeleting ? "rgba(239,68,68,0.4)" : "#f87171", fontWeight: 800, fontSize: "0.82rem", borderRadius: 10, padding: "10px 14px", cursor: bulkDeleting ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}
          >
        ...
        <div style={{ background: relinkResult.linked > 0 ? "rgba(80,200,120,0.08)" : "rgba(255,180,0,0.08)", border: `1px solid ${relinkResult.linked > 0 ? "rgba(80,200,120,0.3)" : "rgba(255,180,0,0.3)"}`, borderRadius: 10, padding: "10px 16px", marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.82rem", color: C.text }}>
          <span>
            {relinkResult.linked > 0
              ? `✅ Linked ${relinkResult.linked} activit${relinkResult.linked === 1 ? "y" : "ies"} to accounts. ${relinkResult.skipped > 0 ? `${relinkResult.skipped} could not be matched (no matching account name found).` : ""}`
              : `⚠️ No activities were linked. Make sure you've imported your Accounts first (Admin → Import → Accounts tab), then try again.`}
          </span>
          <button onClick={() => setRelinkResult(null)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: "1rem", padding: "0 4px" }}>×</button>
        </div>
      )}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "28px 40px 1fr 160px 140px 140px 110px 36px", padding: "10px 16px", borderBottom: `1px solid ${C.border}`, fontSize: "0.72rem", fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em" }}>
          <input
            type="checkbox"
        ...
        {loading ? (
          ...
          filtered.map((a, i) => (
            <div
              ...
              style={{
                display: "grid",
                gridTemplateColumns: "28px 40px 1fr 160px 140px 140px 110px 36px",
                padding: "12px 16px",
            ...
            <form onSubmit={handleEdit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div>
                ...
                <select style={{ ...inp, width: "100%" }} value={editForm.hospitalId} onChange={(e) => setEditForm(f => ({ ...f, hospitalId: e.target.value }))}>
                  ...
                  {accounts.map(h => <option key={h.id} value={h.id}>{h.hospitalName}</option>)}
                </select>
              </div>
              <div>
